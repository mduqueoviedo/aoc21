...
const isSmallCaveExtraVisitUsed = (currentPath) => {
    const smallCaves = []
    let visitUsed = false
    ...
    currentPath
        ...
        .forEach((smallCave) => {
            if (smallCaves.includes(smallCave)) {
                visitUsed = true
            } else {
                smallCaves.push(smallCave)
            }
        })
            ...
            caveMap[lastItemInPath]
                .filter((pathOption) =>
                    ...
                    && (
                        ...
                        || (isSmallCave(pathOption) && !isSmallCaveExtraVisitUsed(currentPath))
                    )
                )
                .forEach((pathOption) => {
